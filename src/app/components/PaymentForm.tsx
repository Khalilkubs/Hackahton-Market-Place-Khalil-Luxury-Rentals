...
export default function PaymentForm({ car }: PaymentFormProps) {
  ...
  const validateZipCode = (zipCode: string) => {
    return /^\d{5}(-\d{4})?$/.test(zipCode); // US zip code format
  };

  const validateBillingAddress = (address: string) => {
    return address.trim().length > 5; // Basic validation for address length
  };
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!validateZipCode(zipCode)) errors.zipCode = "Invalid zip code";
    if (!validateBillingAddress(billingAddress)) errors.billingAddress = "Billing address is too short";
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-2">
        ...
        {validationErrors.billingAddress && (
          <span className="text-red-500 text-xs">{validationErrors.billingAddress}</span>
        )}
      </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        <input
          ...
          type="text"
          className={`w-full p-3 border rounded-lg ${
            validationErrors.zipCode ? 'border-red-500' : 'border-gray-200'
          }`}
          placeholder="Enter zip code"
          required
        />
        {validationErrors.zipCode && (
          <span className="text-red-500 text-xs">{validationErrors.zipCode}</span>
        )}
      </div>
